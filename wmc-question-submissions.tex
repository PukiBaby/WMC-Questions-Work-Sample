...
\[
    \sum_{i=1}^n f(A_i) \geq \sum_{i=1}^n \Big( n \cdot f(\overline{a}) \Big)
\]
\[
    \iff \frac{1}{n} \sum_{i=1}^n f(A_i) \geq \sum_{i=1}^n f(\overline{a}) \text{.}
\]
Since
\[
...
\]
for some sequence of primes $\{ p_i \}_{i=1}^M$ and some sequence of positive integers $\{k_i\}_{i=1}^M$. Let $\phi(n)$ be the number of integers $I$ such that $1 \leq I \leq n$ and $\gcd(I, n) = 1$.
...
\begin{equation*}
    \begin{split}
       P \Big( \gcd(I,n) = 1 \Big) &= P \Big( (p_1 \nmid I) \cap (p_2 \nmid I) \cap \cdots \cap (p_M \nmid I) \Big) \\
        &= P \Big( \text{lcm} (p_1, p_2, \dots, p_M) \nmid I \Big) \\
        &= P(p_1 \nmid I) \cdot P(p_2 \nmid I) \cdots P(p_M \nmid I) \\
...
\] 
where $p_1, p_2, \dots$ are primes and $k_1, k_2 \dots \in \mathbb{Z^+}$. Therefore, let
\[
  f(n) =
  \begin{cases}
    1 & \text{if } n = 1, \\
    p_1^{s(k_1)}p_2^{s(k_2)} \cdots & \text{if } n \neq 1 \text{.}
  \end{cases}
\]
We see that $f$ maps every positive rational number to a unique positive integer and every positive integer to a unique rational number because $s$ is a bijection between positive integers and integers.
